...
C    04/01/09: Michael Kevin Hernandez wrote the code to interpolate the
C              netcdf files into pressure levels, with help of Dr. Kevin 
C              Yeh's bilinear vertical interpolation code from his diffpost.
C    04/05/09: Michael Kevin Hernandez added geopotential hieghts [m] into
C              the grib writer.
C
C  Note:
...
C GEOPOT  = PHI = Geopotential Hieght (m)
C
C========================================================================
...
C
        ...
        INTEGER*4 modelhieghtphi, lonphi, latphi
        ...
        REAL*4, ALLOCATABLE, dimension(:) :: datum, pres, temp, mslp
...
200     write(*,'(A34)') '**********************************'
        ...
        write(*,'(A34)') '** Commence pressure interpolation'
...
C
          do k = 1, numbvar, 1
             if (var .eq. varskeep(k)) then
                ...
                ALLOCATE(prestran(londim,latdim,modelhieght))
                CALL NCVGT(ncid, varid, start, count, datum, ncrcode)
                if (var == 'PINT') then
                   pres = datum
                   call prestransform(numpts, modelhieght, pres, londim,
     &                                latdim, prestran)
...
C
     ...
     &                         idd, ihh, imm, var, imap, KGDS, londim, 
     &                         latdim, prestran)
                   DEALLOCATE(datum)
                else if (var == 'T') then
     ...
     &                         latdim, prestran)
        ...
        call geopotential(temp, pres, numptsphi, modelhieghtphi, iyy, 
     &                    imo, idd, ihh, imm, imap, KGDS, lonphi,
     &                    latphi, prestran)
...
CC algorithm to calculate this variable. To get geopotential hieght
CC in meters one must devide it by gravity.
C
     ...
     &              var, imap, KGDS, londim, latdim, prestran)
...
C========================================================================
     ...
     &                  imm, var, imap, KGDS, londim, latdim, prestran)
        ...
        LOGICAL*1, allocatable   :: LB (:)
        REAL*4,    allocatable   :: F(:), Finterp(:), datumtranformed(:)
        REAL*4,    dimension(numpts):: datum
        REAL*4,    dimension(londim,latdim,43):: prestran
...
C========================================================================
     ...
     &                          datumtransformed)
        ...
        REAL*4,    dimension(43*nx*ny):: pres 
        REAL*4,    dimension(nx,ny,nz):: threeDarray
        REAL*4,    dimension(nx,ny,41):: datumtwo
        REAL*4,    dimension(nx,ny,43):: pm
        REAL*4,    dimension(41):: plev
...
C
        ...
        KPDS(6)  = 100  ! 100 for isobaric pressure in hectoPascals
        KPDS(7)  = zed  ! if KPDS(6) = 100 then zed which is the 
